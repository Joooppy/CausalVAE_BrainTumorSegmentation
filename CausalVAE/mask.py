...
class MaskLayer(nn.Module):
	def __init__(self, z_dim, concept=3, z2_dim=3):
		super().__init__()
		self.z_dim = z_dim
		self.z2_dim = z2_dim
	...
	def mix(self, z):
		zy = z.view(-1, self.concept*self.z2_dim).float()
		if self.z2_dim == 1:
...
class CausalLayer(nn.Module):
	def __init__(self, z_dim, concept=3,z1_dim=3):
		super().__init__()
		self.z_dim = z_dim
		self.z1_dim = z1_dim
		self.concept = concept
...
class DagLayer(nn.Linear):
    def __init__(self, in_features, out_features,i = False, bias=False, initial=True):
        ...
        self.a = self.a
        #self.initial_dag = torch.tensor(np.array([[0.0000001, 0.5, 0.5],
        #                           [0.5, 0.0000001, 0.5],
        #                           [0.5, 0.5, 0.0000001]]), dtype=torch.float32)
        
    	
        if initial:
            print("Initializing DAG matrix with custom initial values.")
            #self.a = self.initial_dag.clone().detach()
            #self.a = self.initial_dag
            self.a[0][1], self.a[0][2], self.a[1][2] = 1,1,1
            self.a[1][0], self.a[2][0], self.a[2][1] = 1,1,1
    ...
    def mask_u(self,x):
        x = x.float()
        self.B = self.A.float() if self.A.dtype != torch.float else self.A
        #if self.i:
        #    x = x.view(-1, x.size()[1], 1)
        #    x = torch.matmul((self.B+0.5).t().int().float(), x)
        #    return x

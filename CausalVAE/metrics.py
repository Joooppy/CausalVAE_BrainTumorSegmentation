...
class CombinedLoss(_Loss):
    ...
    def forward(self, y_pred, y_true, inputs, rec_image, L_loss):
        #est_mean, est_std = (y_mid[:, :128], y_mid[:, 128:])
        seg_pred, seg_truth = (y_pred[:,:3,:,:,:], y_true[:,:3,:,:,:]) # problem
        vae_pred, vae_truth = (rec_image, inputs)
        ...
        if self.focal_enable:
            ...
            combined_loss = dice_loss + self.k1 * l2_loss + self.k2 * L_loss + focal_loss
        else:
            combined_loss = dice_loss + self.k1 * l2_loss + self.k2 * L_loss
        #print("dice_loss:%.4f, L2_loss:%.4f, L_Loss:%.4f, combined_loss:%.4f"%(dice_loss,l2_loss,kl_div,combined_loss))

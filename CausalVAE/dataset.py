...
def calculate_label_ratios(ed_sum, ncr_sum, et_sum, brain_mask):
    """
    Calculate the proportion of each label (ed, ncr, et) within the brain mask.

    Args:
    ed_label (np.ndarray): Binary mask for peritumoral edema.
    ncr_label (np.ndarray): Binary mask for necrotic and non-enhancing tumor core.
    et_label (np.ndarray): Binary mask for GD-enhancing tumor.
    brain_mask (np.ndarray): Binary mask of the brain area.

    Returns:
    list: Proportions of each label within the brain mask.
    """

    
    brain_mask_area = np.sum(brain_mask)
    print("brain_mask_area: ", brain_mask_area)
    ed_ratio = ed_sum / brain_mask_area
    ncr_ratio = ncr_sum / brain_mask_area
    et_ratio = et_sum / brain_mask_area
    #tum_ratio = ed_ratio + ncr_ratio + et_ratio


    return [ed_ratio, ncr_ratio, et_ratio]


def calculate_label_distribution(ed_sum, ncr_sum, et_sum, brain_mask):
    """
    Calculate the label distribution (proportion of each label - ed, ncr, et) within the brain mask.

    Args:
    ed_sum (np.ndarray): Sum of binary mask values for peritumoral edema.
    ncr_sum (np.ndarray): Sum of binary mask values for necrotic and non-enhancing tumor core.
    et_sum (np.ndarray): Sum of binary mask values for GD-enhancing tumor.
    brain_mask (np.ndarray): Binary mask of the brain area.

    Returns:
    list: Normalized label distribution (proportion of each label within the brain mask, summing to 1).
    """

    # Calculate the total area of the brain mask
    brain_mask_area = np.sum(brain_mask)
    if brain_mask_area == 0:
        raise ValueError("Brain mask area is zero. Cannot calculate ratios.")
    ...
    # Combine the ratios into a list
    ratios = [ed_ratio, ncr_ratio, et_ratio]

    # Calculate the total ratio
    total_ratio = sum(ratios)
    
    # Normalize the ratios to form a label distribution (ensures sum is 1)
    if total_ratio == 0:
        # If there are no tumor labels, return uniform distribution or adjust as needed
        label_distribution = [1.0 / len(ratios)] * len(ratios)  # uniform distribution
    else:
        label_distribution = [r / total_ratio for r in ratios]
...
def preprocess_label(img, single_label=None):
    ...
    Calculates sums of masks for later ratio calculation
    """
    ...
    ncr_sum = np.sum(ncr)
    et_sum = np.sum(et)
    print("ed total: ",ed_sum)
    print("ncr total: ", ncr_sum)
    print("et total: ", et_sum)
    ...
    return img[np.newaxis, :], ed_sum, ncr_sum, et_sum


class BratsDataset(Dataset):
    """
    Prepare image samples to be applicable for the model
    """
    ...
    def __getitem__(self, index):
        ...
        # label brain mask
        label_nonzero_masks = [i != 0 for i in imgs_npy[:-1]]
        label_brain_mask = np.zeros(imgs_npy.shape[1:], dtype=bool)
        for chl in range(len(label_nonzero_masks)):
            label_brain_mask = label_brain_mask | label_nonzero_masks[chl]
        print(label_brain_mask.shape)        
        ...
        if self.phase == "train":
            ...
            #label_brain_mask_cropped = random_crop(label_brain_mask, crop_size=self.input_shape[2:])
            ...
            #label_brain_mask_cropped = validation_time_crop(label_brain_mask, crop_size=self.input_shape[2:])
            
        elif self.phase == "evaluation":
            cur_image_with_label = imgs_npy.copy()
            #label_brain_mask_cropped = label_brain_mask.copy()

        if self.phase == "validate" or self.phase == "train" or self.phase == "evaluation":
            inp_data = cur_image_with_label[:-1]
            seg_label, ed_sum, ncr_sum, et_sum = preprocess_label(cur_image_with_label[-1], self.seg_label)
            
            label_distr = calculate_label_distribution(ed_sum, ncr_sum, et_sum, label_brain_mask)
            ...
            return np.array(inp_data), np.array(final_label), np.array(label_distr)
